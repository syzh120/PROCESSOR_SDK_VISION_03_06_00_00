...
#include <ti/drv/stw_lld/fatlib/fatfs/ff.h>			/* Declarations of FatFs API */
...
/* Defined externally as a workaround to not having to do virtual address
   translations in this file */
extern void mem_cpy_dma (void* dst, const void* src, unsigned int cnt);
	...
	*br = 0;	/* Clear read byte counter */
	...
	for ( ;  btr;								/* Repeat until all data read */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				...
				if (fp->fs->wflag && fp->fs->winsect - sect < cc)
					mem_cpy_dma(rbuff + ((fp->fs->winsect - sect) * SS(fp->fs)), fp->fs->win, SS(fp->fs));
#else
				if ((fp->flag & FA__DIRTY) && fp->dsect - sect < cc)
					mem_cpy_dma(rbuff + ((fp->dsect - sect) * SS(fp->fs)), fp->buf, SS(fp->fs));
#endif
#endif
		...
		if (move_window(fp->fs, fp->dsect) != FR_OK)		/* Move sector window */
			ABORT(fp->fs, FR_DISK_ERR);
		mem_cpy_dma(rbuff, &fp->fs->win[fp->fptr % SS(fp->fs)], rcnt);	/* Pick partial sector */
#else
		mem_cpy_dma(rbuff, &fp->buf[fp->fptr % SS(fp->fs)], rcnt);	/* Pick partial sector */
#endif
	}
...
/*-----------------------------------------------------------------------*/
/* Added by [SN]                                                         */
/* Put a formatted string to the file, function takes a variable list    */
/* argument                                                              */
/*-----------------------------------------------------------------------*/
