...
function createMake(makefile)
{
    ...
    if(makefile==undefined)
    {
      ...
      if(delivery_type=="src")
      {
        ...
        Command += " -ccs.buildTarget \'.libraries,e66'";
 ...
 */
 ...
 function checktargetrule(target,makefile)
 {
   ...
   if(found == false)
   {
     ...
     if(target=="86") {
        makefile.writeLine("else # Linux Build Environment");                
        makefile.writeLine("\n# all,"+target+"l rule");                
        makefile.writeLine("all,"+target+"l: .executables,"+target+"l");           
        makefile.writeLine(".executables,"+target+"l: .projects,"+target+"l");
        makefile.writeLine(".projects,"+target+"l: .libraries,"+target+"l");
        makefile.writeLine(".libraries,"+target+"l:\n");            
     }
             ...
             if(files[ID].cmodel==true)
             {
              if(cmlib=="")
              {
                  ...
                  checktargetrule(targs[k].suffix,makefilelocal);
                    ...
                    if(String(targs[k].suffix).match("86")) {
                    ...
                    if(targs[k].suffix=="86") {
                      makefilelocal.writeLine("\nelse # Linux Build Environment");                
                      makefilelocal.writeLine("\n# Make rule to create "+cmlib+"l library");
                      makefilelocal.writeLine(".libraries,"+targs[k].suffix+"l .libraries: "+ cmlib+"l");
                      makefilelocal.writeLine(cmlib+"l: FORCE\n\t$(MAKE) -f "+cmlib+"l.mk $@");
                      makefilelocal.writeLine("# Rule to clean "+cmlib+"l library");                                          
                      makefilelocal.writeLine("clean ::\n\t$(RM) "+ cmlib+"l");                                        
                    }
             ...
             else
             {
                if(cfiles.length > 0 || bundletype == "kernel" )
                {
                  if(clib=="")
                  {
                        ...
                        if(targs[k].suffix=="86") {
                          makefilelocal.writeLine("\nelse # Linux Build Environment");                
                          makefilelocal.writeLine("\n# Make rule to create "+clib+"l library");
                          makefilelocal.writeLine(".libraries,"+targs[k].suffix+"l .libraries: "+ clib+"l");
                          makefilelocal.writeLine(clib+"l: FORCE\n\t$(MAKE) -f "+clib+"l.mk $@");
                          makefilelocal.writeLine("# Rule to clean "+clib+"l library");                                          
                          makefilelocal.writeLine("clean ::\n\t$(RM) "+ clib+"l");                                        
                        }
                ...
                if(bundletype != "kernel" && (afiles.length > 0 || lafiles.length > 0 || objfiles.length > 0))
               {
                 if(alib=="")
                 {
                      ...
                      if(targs[k].suffix=="86") {
                          makefilelocal.writeLine("\nelse # Linux Build Environment");                
                          makefilelocal.writeLine("\n# Make rule to create "+alib+"l library");
                          makefilelocal.writeLine(".libraries,"+targs[k].suffix+"l .libraries: "+ alib+"l");
                          makefilelocal.writeLine(alib+"l: FORCE\n\t$(MAKE) -f "+alib+"l.mk $@");
                          makefilelocal.writeLine("# Rule to clean "+alib+"l library");                                          
                          makefilelocal.writeLine("clean ::\n\t$(RM) "+ alib+"l");                                        
                      }
    ...
    for each(var comp in componentlist)
    {
        ...
        for each(var lib in libs)
        {
          if(comp != "ti.xdais")
          {
            ...
            dependent_libs.push("./../packages/ti/vxlib/"+lib);
          }
        }
    }
...
/*         var pkgExe=Pkg.addExecutable (exeTarget[n].base_directory+"/"+exeTarget[n].name, targs[k], exeTarget[n].platform,
        ...
        for(var z=0; z < pnameHistory.length; z++)
        {
            if(xdc.findFile(pnameHistory[z].replace(/\./g,"/")+"/package.xs") && pnameHistory[z] != "ti.xdais")
            {
                ...
                componentpath = "$(VXLIB_INSTALL_DIR)/ti/vxlib";
        ...
        makefilelocal.writeLine(exename+": FORCE\n\t@$(MAKE) -f "+exename+".mk $@");
         ...
         var objlist="";
          ...
          if(targs[k].suffix=="86") {
             makefilelocal.writeLine("\nelse # Linux Build Environment");                
             makefilelocal.writeLine("\n# Make rule to create "+exename+"l executable");           
             makefilelocal.writeLine(".executables,"+targs[k].suffix+"l .executables: "+ exename+"l");
             makefilelocal.writeLine(exename+": FORCE\n\t@$(MAKE) -f "+exename+"l.mk $@");
...
{ 
  ...
  //Pkg.makeEpilogue += "clean::\n\t-$(RM) .//eclipse//vxlib.xml\n"; 
  //Pkg.makeEpilogue += "clean::\n\t-$(RM) .//eclipse//vxlib_toc.xml\n"; 
  Pkg.makeEpilogue += "clean::\n\t-$(RM) vc110.pdb\n"; 
...
function createProjects()
{
  ...
  for each(var project in ccsProjects)
    ...
    if(createxml==true)
    {
        var slubdirslash  = project.pjtName.lastIndexOf("/");
        var pjtname       = project.pjtName.substr(slubdirslash+1);
        var file = new java.io.File("./package/projects/"+pjtname+"_"+project.targets+".xml");
          var indent = "    ";
        file["delete"]();
          ...
          /* Output Project Parameters */
          out.write(indent + '    <name>'+pjtname+'</name>\n');
          out.write(indent + '    <location>'+project.location+'</location>\n');
        out.write(indent + '    <target>'+project.targets+'</target>\n');
        out.write(indent + '    <buildArtifact>'+project.buildArtifact+'</buildArtifact>\n');
        for each(var file in project.srcFiles)
