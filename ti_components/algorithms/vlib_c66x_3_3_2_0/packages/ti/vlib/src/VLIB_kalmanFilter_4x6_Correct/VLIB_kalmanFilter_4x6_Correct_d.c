...
#include "../VLIB_kalmanFilter_4x6_Predict/VLIB_kalmanFilter_4x6_Predict_cn.h"

#include "VLIB_kalmanFilter_4x6_Correct.h"
#include "VLIB_kalmanFilter_4x6_Correct_cn.h"
#include "VLIB_kalmanFilter_4x6_Correct_idat.h"

/* VLIB_kalmanFilter_4x6_Correct_d:  Test Driver Routine */
void VLIB_kalmanFilter_4x6_Correct_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    kalmanFilter_4x6_Correct_testParams_t   *prm;

    kalmanFilter_4x6_Correct_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_kalmanFilter_4x6_Correct");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( Z && Z_buffer && Residual && Residual_cn && KF && KF_cn ) {
            ...
            for( i=0; i < prm[tpi].numMeasurements; i++ ) {
                ...
                /* kalman predict */
                VLIB_kalmanFilter_4x6_Predict_cn(KF);  // The natC version is used so that the code size doesn't get counted
                ...
                VLIB_kalmanFilter_4x6_Correct(KF, Z, Residual);
                VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_kalmanFilter_4x6_Correct");
    } else {
        VLIB_kalmanFilter_4x6_Correct_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_kalmanFilter_4x6_Correct_d.c                         */
/* ======================================================================== */
