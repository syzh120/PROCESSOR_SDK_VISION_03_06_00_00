...
#include "VLIB_nonMaxSuppress_U32_cn.h"
#include "VLIB_nonMaxSuppress_U32_idat.h"

/* VLIB_nonMaxSuppress_U32_d:  Test Driver Routine */
void VLIB_nonMaxSuppress_U32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    nonMaxSuppressU32_testParams_t   *prm;

    nonMaxSuppressU32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_nonMaxSuppress_U32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        uint32_t   *im           =  (uint32_t *) VLIB_malloc(inp_size * sizeof(uint32_t));
        uint32_t   *buffer       =  (uint32_t *) VLIB_malloc(buf_size * sizeof(uint32_t));
        ...
        if( im && buffer && pixIndex_opt && pixIndex_cn ) {
            ...
            VLIB_fillBuffer(prm[tpi].testPattern,
                            (uint8_t)255,
                            im, prm[tpi].staticIn,
                            prm[tpi].w, prm[tpi].h, prm[tpi].w * sizeof(uint32_t),
                            sizeof(uint32_t), testPatternString);

            /* Test optimized kernel */
            VLIB_profile_start(vlib_KERNEL_OPT);
            VLIB_nonMaxSuppress_U32(im, prm[tpi].w, prm[tpi].h,
            ...
            VLIB_nonMaxSuppress_U32_cn(im, prm[tpi].w, prm[tpi].h,
                                       prm[tpi].filterWidth, prm[tpi].threshold,
                                       buffer, pixIndex_cn);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_nonMaxSuppress_U32");
    } else {
        VLIB_nonMaxSuppress_U32_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_nonMaxSuppress_U32_d.c                               */
/* ======================================================================== */
