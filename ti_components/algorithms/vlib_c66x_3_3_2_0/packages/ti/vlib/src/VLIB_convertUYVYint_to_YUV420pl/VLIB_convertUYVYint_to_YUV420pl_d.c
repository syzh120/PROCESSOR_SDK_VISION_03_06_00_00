...
#include "VLIB_convertUYVYint_to_YUV420pl_cn.h"
#include "VLIB_convertUYVYint_to_YUV420pl_idat.h"

/* VLIB_convertUYVYint_to_YUV420pl_d:  Test Driver Routine */
void VLIB_convertUYVYint_to_YUV420pl_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    UYVYint_to_YUV420pl_testParams_t   *prm;

    UYVYint_to_YUV420pl_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_convertUYVYint_to_YUV420pl");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        uint32_t    out_chroma =  prm[tpi].width * prm[tpi].height / 4;
        ...
        if( yc && y && cr && cb && y_cn && cr_cn && cb_cn ) {
            ...
            VLIB_convertUYVYint_to_YUV420pl(yc, prm[tpi].width, prm[tpi].pitch, prm[tpi].height, y, cr, cb);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_convertUYVYint_to_YUV420pl_cn(yc, prm[tpi].width, prm[tpi].pitch, prm[tpi].height, y_cn, cr_cn, cb_cn);
            VLIB_profile_stop();
            ...
            VLIB_formula_add_test(out_luma, prm[tpi].height, NULL, fail, desc, NULL);
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_convertUYVYint_to_YUV420pl");
    } else {
        VLIB_convertUYVYint_to_YUV420pl_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_convertUYVYint_to_YUV420pl_d.c                       */
/* ======================================================================== */
