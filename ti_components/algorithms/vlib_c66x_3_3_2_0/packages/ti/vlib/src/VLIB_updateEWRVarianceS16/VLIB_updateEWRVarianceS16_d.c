...
#include "VLIB_updateEWRVarianceS16_cn.h"
#include "VLIB_updateEWRVarianceS16_idat.h"

/* VLIB_updateEWRVarianceS16_d:  Test Driver Routine */
void VLIB_updateEWRVarianceS16_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    updateEWRVarianceS16_testParams_t   *prm;

    updateEWRVarianceS16_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_updateEWRVarianceS16");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( newLuma && prevLuma && mask32packed && runningMean && runningVar && runningVar_cn ) {
            ...
            /* The natC version of this vlib function is used so that the code size doesn't get counted in this test */
            VLIB_initVarWithConstS16_cn(runningVar, prm[tpi].constant, prm[tpi].pixelCount);
            VLIB_initVarWithConstS16_cn(runningVar_cn, prm[tpi].constant, prm[tpi].pixelCount);

            /* Test optimized kernel */
            VLIB_profile_start(vlib_KERNEL_OPT);
            VLIB_updateEWRVarianceS16(runningVar, runningMean, newLuma, mask32packed, prm[tpi].weight, prm[tpi].pixelCount);
            VLIB_profile_stop();


            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_updateEWRVarianceS16_cn(runningVar_cn, runningMean, newLuma, mask32packed, prm[tpi].weight, prm[tpi].pixelCount);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_updateEWRVarianceS16");
    } else {
        VLIB_updateEWRVarianceS16_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_updateEWRVarianceS16_d.c                             */
/* ======================================================================== */
