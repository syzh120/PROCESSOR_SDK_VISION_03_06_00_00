...
#include "VLIB_extractLumaFromUYUV_cn.h"
#include "VLIB_extractLumaFromUYUV_idat.h"

/* VLIB_extractLumaFromUYUV_d:  Test Driver Routine */
void VLIB_extractLumaFromUYUV_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    extractLumaFromUYUV_testParams_t   *prm;

    extractLumaFromUYUV_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_extractLumaFromUYUV");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( pInUYVY && pOutY && pOutY_cn ) {
            ...
            VLIB_fillBuffer(prm[tpi].testPattern,
                            (uint8_t)255,
                            pInUYVY, prm[tpi].staticIn,
                            prm[tpi].inCols, prm[tpi].inRows, prm[tpi].inPitch * 2,
                            sizeof(uint16_t), testPatternString);

            /* Test optimized kernel */
            VLIB_profile_start(vlib_KERNEL_OPT);
            VLIB_extractLumaFromUYUV(pInUYVY, prm[tpi].inCols, prm[tpi].inPitch, prm[tpi].inRows, pOutY);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_extractLumaFromUYUV_cn(pInUYVY, prm[tpi].inCols, prm[tpi].inPitch, prm[tpi].inRows, pOutY_cn);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_extractLumaFromUYUV");
    } else {
        VLIB_extractLumaFromUYUV_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_extractLumaFromUYUV_d.c                              */
/* ======================================================================== */
