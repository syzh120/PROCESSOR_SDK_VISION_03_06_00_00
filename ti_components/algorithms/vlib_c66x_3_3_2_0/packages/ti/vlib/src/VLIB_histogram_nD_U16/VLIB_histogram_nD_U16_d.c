...
#include "VLIB_histogram_nD_U16_cn.h"
#include "VLIB_histogram_nD_U16_idat.h"

/* VLIB_histogram_nD_U16_d:  Test Driver Routine */
void VLIB_histogram_nD_U16_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    histogram_nD_U16_testParams_t   *prm;

    histogram_nD_U16_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(4, "VLIB_histogram_nD_U16");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( X && numBins && normVals && scrap ) {
            ...
            if( H && H_cn && H1 ) {
                ...
                VLIB_histogram_nD_U16(X, prm[tpi].numX, prm[tpi].dimX, prm[tpi].binWeight, numBins, normVals, scrap, H1, H);
                VLIB_profile_stop();
                ...
                VLIB_histogram_nD_U16_cn(X, prm[tpi].numX, prm[tpi].dimX, prm[tpi].binWeight, numBins, normVals, scrap, H1, H_cn);
                VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_histogram_nD_U16");
    } else {
        VLIB_histogram_nD_U16_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_histogram_nD_U16_d.c                                */
/* ======================================================================== */
