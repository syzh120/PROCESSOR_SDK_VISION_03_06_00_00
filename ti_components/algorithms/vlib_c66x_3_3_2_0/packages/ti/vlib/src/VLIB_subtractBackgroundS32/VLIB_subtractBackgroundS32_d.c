...
#include "VLIB_subtractBackgroundS32_cn.h"
#include "VLIB_subtractBackgroundS32_idat.h"

/* VLIB_subtractBackgroundS32_d:  Test Driver Routine */
void VLIB_subtractBackgroundS32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    subtractBackgroundS32_testParams_t   *prm;

    subtractBackgroundS32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_subtractBackgroundS32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( newLuma && prevLuma && runningMean && runningVar && mask32packed && mask32packed_cn ) {
            ...
            /* The natC version of this vlib function is used so that the code size doesn't get counted in this test */
            VLIB_initVarWithConstS32_cn(runningVar, prm[tpi].thresholdGlobal, prm[tpi].PixelCount);

            /* Test optimized kernel */
            VLIB_profile_start(vlib_KERNEL_OPT);
            VLIB_subtractBackgroundS32(mask32packed, newLuma, runningMean, runningVar, prm[tpi].thresholdGlobal, prm[tpi].thresholdFactor, prm[tpi].PixelCount);
            VLIB_profile_stop();


            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_subtractBackgroundS32_cn(mask32packed_cn, newLuma, runningMean, runningVar, prm[tpi].thresholdGlobal, prm[tpi].thresholdFactor, prm[tpi].PixelCount);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_subtractBackgroundS32");
    } else {
        VLIB_subtractBackgroundS32_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_subtractBackgroundS32_d.c                            */
/* ======================================================================== */
