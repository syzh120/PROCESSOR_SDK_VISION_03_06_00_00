...
#include "VLIB_updateUWRMeanS16_cn.h"
#include "VLIB_updateUWRMeanS16_idat.h"

/* VLIB_updateUWRMeanS16_d:  Test Driver Routine */
void VLIB_updateUWRMeanS16_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    updateUWRMeanS16_testParams_t   *prm;

    updateUWRMeanS16_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_updateUWRMeanS16");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        uint8_t    *oldestData          =  (uint8_t *) VLIB_malloc(prm[tpi].pixelCount * sizeof(uint8_t));
        ...
        int16_t    *updatedMean_cn      =  (int16_t *) malloc(prm[tpi].pixelCount * sizeof(int16_t));

        /* Only run the test if the buffer allocations fit in the heap */
        if( prevLuma && previousMean && newestData && oldestData &&
            newestMask32packed && oldestMask32packed && updatedMean && updatedMean_cn ) {
            ...
            if( prm[tpi].newestData ) {
                memcpy(newestData, prm[tpi].newestData, prm[tpi].pixelCount);
            } else {
                VLIB_fillBuffer(RANDOM,
                                (uint8_t)1,
            ...
            if( prm[tpi].oldestData ) {
                memcpy(oldestData, prm[tpi].oldestData, prm[tpi].pixelCount);
            } else {
                VLIB_fillBuffer(RANDOM,
                                (uint8_t)1,
            ...
            VLIB_updateUWRMeanS16(updatedMean, previousMean, newestData, oldestData, newestMask32packed, oldestMask32packed,
            ...
            VLIB_updateUWRMeanS16_cn(updatedMean_cn, previousMean, newestData, oldestData, newestMask32packed, oldestMask32packed,
                                     prm[tpi].pixelCount, prm[tpi].frameCount);
            VLIB_profile_stop();


            /* Compare natural C Output and Optimized Output */
            status_nat_vs_int = VLIB_compare_mem((void *) updatedMean, (void *)updatedMean_cn, prm[tpi].pixelCount * sizeof(updatedMean_cn[0]));

            /* If static output is available, then additionally compares natural C output with static reference output data */
            if( prm[tpi].staticOutMean != NULL ) {
                status_nat_vs_ref = VLIB_compare_mem((void *) prm[tpi].staticOutMean, (void *)updatedMean_cn, prm[tpi].pixelCount * sizeof(updatedMean_cn[0]));
            }
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_updateUWRMeanS16");
    } else {
        VLIB_updateUWRMeanS16_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_updateUWRMeanS16_d.c                                 */
/* ======================================================================== */
