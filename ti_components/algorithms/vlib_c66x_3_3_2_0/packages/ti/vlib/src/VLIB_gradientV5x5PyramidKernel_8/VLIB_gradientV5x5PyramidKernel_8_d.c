...
#include "VLIB_gradientV5x5PyramidKernel_8_cn.h"
#include "VLIB_gradientV5x5PyramidKernel_8_idat.h"

/* VLIB_gradientH5x5PyramidKernel_8_d:  Test Driver Routine */
void VLIB_gradientV5x5PyramidKernel_8_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    gradientV5x5Pyramid8_testParams_t   *prm;

    gradientV5x5Pyramid8_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_gradientV5x5PyramidKernel_8");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( pIn && pB && pOut && pOut_cn ) {
            ...
            VLIB_gradientV5x5PyramidKernel_8(pIn, pB, prm[tpi].width, prm[tpi].pitch, prm[tpi].height, pOut);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_gradientV5x5PyramidKernel_8_cn(pIn, pB, prm[tpi].width, prm[tpi].pitch, prm[tpi].height, pOut_cn);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_gradientV5x5PyramidKernel_8");
    } else {
        VLIB_gradientV5x5PyramidKernel_8_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_gradientV5x5PyramidKernel_8_d.c                      */
/* ======================================================================== */
