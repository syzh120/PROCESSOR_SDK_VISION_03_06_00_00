...
#include "../VLIB_kalmanFilter_2x4_Correct_F32/VLIB_kalmanFilter_2x4_Correct_F32_cn.h"

#include "VLIB_kalmanFilter_2x4_Predict_F32.h"
#include "VLIB_kalmanFilter_2x4_Predict_F32_cn.h"
#include "VLIB_kalmanFilter_2x4_Predict_F32_idat.h"

/* VLIB_kalmanFilter_2x4_Predict_d:  Test Driver Routine */
void VLIB_kalmanFilter_2x4_Predict_F32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    kalmanFilter_2x4_Predict_F32_testParams_t   *prm;

    kalmanFilter_2x4_Predict_F32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_kalmanFilter_2x4_Predict_F32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        VLIB_F32                    *Z           = (VLIB_F32 *) VLIB_memalign(8, 2 * sizeof(VLIB_F32));
        ...
        VLIB_kalmanFilter_2x4_F32   *KF          = (VLIB_kalmanFilter_2x4_F32 *) VLIB_memalign(8, sizeof(VLIB_kalmanFilter_2x4_F32));
        VLIB_kalmanFilter_2x4_F32   *KF_cn       = (VLIB_kalmanFilter_2x4_F32 *) memalign(8, sizeof(VLIB_kalmanFilter_2x4_F32));
        ...
        if( Z && Z_buffer && Residual && Residual_cn && KF && KF_cn ) {
            ...
            memset(KF, 0, sizeof(VLIB_kalmanFilter_2x4_F32));
            memset(KF_cn, 0, sizeof(VLIB_kalmanFilter_2x4_F32));
            ...
            for( i=0; i < prm[tpi].numMeasurements; i++ ) {
                ...
                VLIB_kalmanFilter_2x4_Predict_F32(KF);
                VLIB_profile_stop();

                /* Test _cn kernel */
                VLIB_profile_start(vlib_KERNEL_CN);
                VLIB_kalmanFilter_2x4_Predict_F32_cn(KF_cn);
                VLIB_profile_stop();
                ...
                VLIB_kalmanFilter_2x4_Correct_F32_cn(KF, Z, Residual); // The natC version is used so that the code size doesn't get counted
                VLIB_kalmanFilter_2x4_Correct_F32_cn(KF_cn, Z, Residual_cn);
                ...
                for( j=0; j < 4; j++ ) {
                    ...
                    if( prm[tpi].staticOut != NULL ) {
                        if( fabs(KF_cn->state[j] - prm[tpi].staticOut[(i * 4) + j]) > (0.1)) {
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_kalmanFilter_2x4_Predict_F32");
    } else {
        VLIB_kalmanFilter_2x4_Predict_F32_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_kalmanFilter_2x4_Predict_F32_d.c                     */
/* ======================================================================== */
