...
#include "VLIB_recursiveFilterHoriz1stOrderS16_cn.h"
...
/* VLIB_recursiveFilterHoriz1stOrderS16_d:  Test Driver Routine */
void VLIB_recursiveFilterHoriz1stOrderS16_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    recursiveFilterHoriz1stOrderS16_testParams_t   *prm;

    recursiveFilterHoriz1stOrderS16_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_recursiveFilterHoriz1stOrderS16");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        int16_t   *scratch = (int16_t *) VLIB_malloc(8 * prm[tpi].width * sizeof(int16_t));
        int16_t   *boundaryLeft  = (int16_t *) VLIB_malloc(prm[tpi].height * sizeof(int16_t));
        int16_t   *boundaryRight = (int16_t *) VLIB_malloc(prm[tpi].height * sizeof(int16_t));

        /* Only run the test if the buffer allocations fit in the heap */
        if( in && out && out_cn && scratch && boundaryLeft && boundaryRight ) {

            int32_t    fail;
            int16_t   *pBoundaryLeft = NULL, *pBoundaryRight = NULL;
            ...
            if( prm[tpi].boundaryLeft ) {
                memcpy(boundaryLeft, prm[tpi].boundaryLeft, prm[tpi].height * sizeof(boundaryLeft[0]));
                pBoundaryLeft = boundaryLeft;
            }
            if( prm[tpi].boundaryRight ) {
                memcpy(boundaryRight, prm[tpi].boundaryRight, prm[tpi].height * sizeof(boundaryRight[0]));
                pBoundaryRight = boundaryRight;
            }

            /* Test optimized kernel */
            VLIB_profile_start(vlib_KERNEL_OPT);
            VLIB_recursiveFilterHoriz1stOrderS16(out, in, prm[tpi].width, prm[tpi].height, prm[tpi].weight,
                                                 pBoundaryLeft, pBoundaryRight, scratch);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_recursiveFilterHoriz1stOrderS16_cn(out_cn, in, prm[tpi].width, prm[tpi].height, prm[tpi].weight,
                                                    pBoundaryLeft, pBoundaryRight, scratch);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_recursiveFilterHoriz1stOrderS16");
    } else {
        VLIB_recursiveFilterHoriz1stOrderS16_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_recursiveFilterHoriz1stOrderS16_d.c                                      */
/* ======================================================================== */
