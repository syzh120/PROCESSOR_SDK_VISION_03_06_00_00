...
#include "VLIB_nonMaxSuppress_5x5_S16_cn.h"
#include "VLIB_nonMaxSuppress_5x5_S16_idat.h"

/* VLIB_nonMaxSuppress_5x5_S16_d:  Test Driver Routine */
void VLIB_nonMaxSuppress_5x5_S16_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    nonMaxSuppress_5x5_S16_testParams_t   *prm;

    nonMaxSuppress_5x5_S16_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_nonMaxSuppress_5x5_S16");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        uint32_t    out_size = prm[tpi].width * (prm[tpi].height - 4); /* Last 4 rows are not written to */
        ...
        if( im && out && out_cn ) {
            ...
            VLIB_nonMaxSuppress_5x5_S16(im, prm[tpi].width, prm[tpi].height, prm[tpi].threshold, out);
            VLIB_profile_stop();


            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_nonMaxSuppress_5x5_S16_cn(im, prm[tpi].width, prm[tpi].height, prm[tpi].threshold, out_cn);
            VLIB_profile_stop();

            /* Compare natural C Output and Optimized Output */
            /* Only valid region should be compared since the natc doesn't write to non-valid region, but opt version does */
            for( i = 0; i < prm[tpi].height - 4; i++ ) {
                for( j = 0; j < prm[tpi].width - 4; j++ ) {
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_nonMaxSuppress_5x5_S16");
    } else {
        VLIB_nonMaxSuppress_5x5_S16_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_nonMaxSuppress_5x5_S16_d.c                           */
/* ======================================================================== */
