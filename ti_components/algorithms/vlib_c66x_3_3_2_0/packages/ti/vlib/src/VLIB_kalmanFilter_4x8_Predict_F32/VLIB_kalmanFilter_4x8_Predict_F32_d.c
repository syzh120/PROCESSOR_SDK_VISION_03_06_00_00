...
#include "../VLIB_kalmanFilter_4x8_Correct_F32/VLIB_kalmanFilter_4x8_Correct_F32_cn.h"

#include "VLIB_kalmanFilter_4x8_Predict_F32.h"
#include "VLIB_kalmanFilter_4x8_Predict_F32_cn.h"
#include "VLIB_kalmanFilter_4x8_Predict_F32_idat.h"

/* VLIB_kalmanFilter_4x8_Predict_F32_d:  Test Driver Routine */
void VLIB_kalmanFilter_4x8_Predict_F32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi; /* test parameter index */

    /* Test Parameters */
    VLIB_kalmanFilter_4x8_Predict_F32_testParams_t   *prm;

    VLIB_kalmanFilter_4x8_Predict_F32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_kalmanFilter_4x8_Predict_F32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        VLIB_kalmanFilter_4x8_F32   *KF          = (VLIB_kalmanFilter_4x8_F32 *) VLIB_memalign(8, sizeof(VLIB_kalmanFilter_4x8_F32));
        VLIB_kalmanFilter_4x8_F32   *KF_cn       = (VLIB_kalmanFilter_4x8_F32 *) malloc(sizeof(VLIB_kalmanFilter_4x8_F32));
        ...
        if( Z && Z_buffer && Residual && Residual_cn && KF && KF_cn ) {
            ...
            memset(KF_cn, 0, sizeof(VLIB_kalmanFilter_4x8_F32));
            ...
            for( i=0; i < prm[tpi].numMeasurements; i++ ) {
                ...
                VLIB_kalmanFilter_4x8_Predict_F32_cn(KF_cn);
                VLIB_profile_stop();

                /* get measurement */
                Z[0] = Z_buffer[i * mD_4x8 + 0];
                Z[1] = Z_buffer[i * mD_4x8 + 1];
                Z[2] = Z_buffer[i * mD_4x8 + 2];
                Z[3] = Z_buffer[i * mD_4x8 + 3];

                VLIB_kalmanFilter_4x8_Correct_F32_cn(KF, Z, Residual); // The natC version is used so that the code size doesn't get counted
                VLIB_kalmanFilter_4x8_Correct_F32_cn(KF_cn, Z, Residual_cn);
...
{
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_kalmanFilter_4x8_Predict_F32");
    } else {
        VLIB_kalmanFilter_4x8_Predict_F32_d(0);
    }
    return (0);
}
...
/* ======================================================================== */
/*  End of file:  VLIB_kalmanFilter_4x8_predict_F32_d.c                     */
/* ======================================================================== */
