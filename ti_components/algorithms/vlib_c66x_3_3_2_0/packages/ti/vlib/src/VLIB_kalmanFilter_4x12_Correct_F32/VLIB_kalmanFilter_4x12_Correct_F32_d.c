...
#include "../VLIB_kalmanFilter_4x12_Predict_F32/VLIB_kalmanFilter_4x12_Predict_F32_cn.h"

#include "VLIB_kalmanFilter_4x12_Correct_F32.h"
#include "VLIB_kalmanFilter_4x12_Correct_F32_cn.h"
#include "VLIB_kalmanFilter_4x12_Correct_F32_idat.h"

/* VLIB_kalmanFilter_4x12_Correct_F32_d:  Test Driver Routine */
void VLIB_kalmanFilter_4x12_Correct_F32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi; /* test parameter index */

    /* Test Parameters */
    VLIB_kalmanFilter_4x12_Correct_F32_testParams_t   *prm;

    VLIB_kalmanFilter_4x12_Correct_F32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_kalmanFilter_4x12_Correct_F32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        VLIB_kalmanFilter_4x12_F32   *KF          = (VLIB_kalmanFilter_4x12_F32 *) VLIB_memalign(8, sizeof(VLIB_kalmanFilter_4x12_F32));
        VLIB_kalmanFilter_4x12_F32   *KF_cn       = (VLIB_kalmanFilter_4x12_F32 *) malloc(sizeof(VLIB_kalmanFilter_4x12_F32));
        ...
        if( Z && Z_buffer && Residual && Residual_cn && KF && KF_cn ) {
            ...
            memset(KF, 0, sizeof(VLIB_kalmanFilter_4x12_F32));
            memset(KF_cn, 0, sizeof(VLIB_kalmanFilter_4x12_F32));
            ...
            KF->state[6] = KF_cn->state[6] = 0;
            KF->state[7] = KF_cn->state[7] = 0;
            KF->state[8]  = KF_cn->state[8]  = 0;
            KF->state[9]  = KF_cn->state[9]  = 0;
            KF->state[10] = KF_cn->state[10] = 0;
            KF->state[11] = KF_cn->state[11] = 0;
            ...
            for( i=0; i < prm[tpi].numMeasurements; i++ ) {
                ...
                /* kalman predict */
                VLIB_kalmanFilter_4x12_Predict_F32_cn(KF); // The natC version is used so that the code size doesn't get counted
                VLIB_kalmanFilter_4x12_Predict_F32_cn(KF_cn);

                /* get measurement */
                Z[0] = Z_buffer[i * mD_4x12 + 0];
                Z[1] = Z_buffer[i * mD_4x12 + 1];
                Z[2] = Z_buffer[i * mD_4x12 + 2];
                Z[3] = Z_buffer[i * mD_4x12 + 3];

                /* Test optimized kernel */
                VLIB_profile_start(vlib_KERNEL_OPT);
                VLIB_kalmanFilter_4x12_Correct_F32(KF, Z, Residual);
                VLIB_profile_stop();

                /* Test _cn kernel */
                VLIB_profile_start(vlib_KERNEL_CN);
                VLIB_kalmanFilter_4x12_Correct_F32_cn(KF_cn, Z, Residual_cn);
                VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_kalmanFilter_4x12_Correct_F32");
    } else {
        VLIB_kalmanFilter_4x12_Correct_F32_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_kalmanFilter_4x12_Correct_F32_d.c                    */
/* ======================================================================== */
