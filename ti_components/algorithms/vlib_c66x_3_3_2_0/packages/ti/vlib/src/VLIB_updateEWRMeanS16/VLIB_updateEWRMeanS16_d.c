...
#include "VLIB_updateEWRMeanS16_cn.h"
#include "VLIB_updateEWRMeanS16_idat.h"

/* VLIB_updateEWRMeanS16_d:  Test Driver Routine */
void VLIB_updateEWRMeanS16_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    updateEWRMeanS16_testParams_t   *prm;

    updateEWRMeanS16_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_updateEWRMeanS16");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( newLuma && prevLuma && mask32packed && runningMean && runningMean_cn ) {
            ...
            VLIB_initMeanWithLumaS16_cn(runningMean_cn, prevLuma, prm[tpi].pixelCount);

            /* Test optimized kernel */
            VLIB_profile_start(vlib_KERNEL_OPT);
            VLIB_updateEWRMeanS16(runningMean, newLuma, mask32packed, prm[tpi].weight, prm[tpi].pixelCount);
            VLIB_profile_stop();


            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_updateEWRMeanS16_cn(runningMean_cn, newLuma, mask32packed, prm[tpi].weight, prm[tpi].pixelCount);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_updateEWRMeanS16");
    } else {
        VLIB_updateEWRMeanS16_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_updateEWRMeanS16_d.c                                 */
/* ======================================================================== */
