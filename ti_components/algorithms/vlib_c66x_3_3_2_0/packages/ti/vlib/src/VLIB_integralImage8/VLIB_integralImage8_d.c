...
#include "VLIB_integralImage8_cn.h"
#include "VLIB_integralImage8_idat.h"

/* VLIB_integralImage8_d:  Test Driver Routine */
void VLIB_integralImage8_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    integralImage8_testParams_t   *prm;

    integralImage8_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_integralImage8");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        uint8_t    *pIn          =  (uint8_t *) VLIB_memalign(8, img_size);
        uint32_t   *pOut         = (uint32_t *) VLIB_memalign(8, img_size * sizeof(uint32_t));
        uint32_t   *pOut_cn      = (uint32_t *) malloc(img_size * sizeof(uint32_t));
        uint32_t   *pLastLine    = (uint32_t *) VLIB_memalign(8, prm[tpi].inCols * sizeof(uint32_t));
        ...
        if( pIn && pOut && pOut_cn && pLastLine && pLastLine_cn ) {
            ...
            VLIB_integralImage8(pIn, prm[tpi].inCols, prm[tpi].inRows, pLastLine, pOut);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_integralImage8_cn(pIn, prm[tpi].inCols, prm[tpi].inRows, pLastLine_cn, pOut_cn);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_integralImage8");
    } else {
        VLIB_integralImage8_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_integralImage8_d.c                                   */
/* ======================================================================== */
