...
#include "VLIB_harrisScore_7x7_S32_cn.h"
#include "VLIB_harrisScore_7x7_S32_idat.h"
...
/* VLIB_harrisScore_7x7_S32_d:  Test Driver Routine */
void VLIB_harrisScore_7x7_S32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    harrisScore_7x7_S32_testParams_t   *prm;

    harrisScore_7x7_S32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_harrisScore_7x7_S32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        int32_t   *outm      =  (int32_t *) VLIB_malloc(out_size * sizeof(int32_t));
        int32_t   *outm_cn   =  (int32_t *) malloc(out_size * sizeof(int32_t));
        ...
        if( input && gradX && gradY && outm && outm_cn && scratch ) {
            ...
            VLIB_harrisScore_7x7_S32(gradX, gradY, prm[tpi].width, prm[tpi].height,
                                     outm, prm[tpi].k, scratch);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_harrisScore_7x7_S32_cn(gradX, gradY, prm[tpi].width, prm[tpi].height,
                                        outm_cn, prm[tpi].k, scratch);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_harrisScore_7x7_S32");
    } else {
        VLIB_harrisScore_7x7_S32_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_harrisScore_7x7_S32_d.c                              */
/* ======================================================================== */
