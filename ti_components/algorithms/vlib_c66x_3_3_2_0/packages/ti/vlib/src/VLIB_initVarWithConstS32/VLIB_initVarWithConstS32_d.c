...
#include "VLIB_initVarWithConstS32_cn.h"
#include "VLIB_initVarWithConstS32_idat.h"

/* VLIB_initVarWithConstS32_d:  Test Driver Routine */
void VLIB_initVarWithConstS32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    initVarWithConstS32_testParams_t   *prm;

    initVarWithConstS32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_initVarWithConstS32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( runningVar && runningVar_cn ) {
            ...
            VLIB_initVarWithConstS32(runningVar, prm[tpi].constVar, prm[tpi].pixelCount);
            VLIB_profile_stop();


            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_initVarWithConstS32_cn(runningVar_cn, prm[tpi].constVar, prm[tpi].pixelCount);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_initVarWithConstS32");
    } else {
        VLIB_initVarWithConstS32_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_initVarWithConstS32_d.c                              */
/* ======================================================================== */
