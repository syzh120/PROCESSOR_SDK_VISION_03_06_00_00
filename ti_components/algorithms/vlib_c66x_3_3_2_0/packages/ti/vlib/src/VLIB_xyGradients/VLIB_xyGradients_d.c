...
#include "VLIB_xyGradients_cn.h"
#include "VLIB_xyGradients_idat.h"

/* VLIB_xyGradients_d:  Test Driver Routine */
void VLIB_xyGradients_d(uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    xyGradients_testParams_t   *prm;

    xyGradients_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_xyGradients");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( pIn && pGradX_opt && pGradY_opt && pGradX_cn && pGradY_cn ) {
            ...
            VLIB_xyGradients(pIn, pGradX_opt, pGradY_opt,
            ...
            VLIB_xyGradients_cn(pIn, pGradX_cn, pGradY_cn,
                                prm[tpi].width, prm[tpi].height);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_xyGradients");
    } else {
        VLIB_xyGradients_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_xyGradients_d.c                          */
/* ======================================================================== */
