...
#include "VLIB_initMeanWithLumaS32_cn.h"
#include "VLIB_initMeanWithLumaS32_idat.h"

/* VLIB_initMeanWithLumaS32_d:  Test Driver Routine */
void VLIB_initMeanWithLumaS32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    initMeanWithLumaS32_testParams_t   *prm;

    initMeanWithLumaS32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_initMeanWithLumaS32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( lumaFrame && runningMean && runningMean_cn ) {
            ...
            VLIB_initMeanWithLumaS32(runningMean, lumaFrame, prm[tpi].pixelCount);
            VLIB_profile_stop();


            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_initMeanWithLumaS32_cn(runningMean_cn, lumaFrame, prm[tpi].pixelCount);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_initMeanWithLumaS32");
    } else {
        VLIB_initMeanWithLumaS32_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_initMeanWithLumaS32_d.c                              */
/* ======================================================================== */
