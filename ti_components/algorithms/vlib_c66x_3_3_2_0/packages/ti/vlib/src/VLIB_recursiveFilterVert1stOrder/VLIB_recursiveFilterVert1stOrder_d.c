...
#include "VLIB_recursiveFilterVert1stOrder_cn.h"
...
/* VLIB_recursiveFilterVert1stOrder_d:  Test Driver Routine */
void VLIB_recursiveFilterVert1stOrder_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    recursiveFilterVert1stOrder_testParams_t   *prm;

    recursiveFilterVert1stOrder_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_recursiveFilterVert1stOrder");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        uint8_t   *scratch = (uint8_t *) VLIB_malloc(8 * prm[tpi].height * sizeof(uint8_t));
        uint8_t   *boundaryTop    = (uint8_t *) VLIB_malloc(prm[tpi].width * sizeof(uint8_t));
        uint8_t   *boundaryBottom = (uint8_t *) VLIB_malloc(prm[tpi].width * sizeof(uint8_t));

        /* Only run the test if the buffer allocations fit in the heap */
        if( in && out && out_cn && scratch && boundaryTop && boundaryBottom ) {

            int32_t    fail;
            uint8_t   *pBoundaryTop = NULL, *pBoundaryBottom = NULL;
            ...
            if( prm[tpi].boundaryTop ) {
                memcpy(boundaryTop, prm[tpi].boundaryTop, prm[tpi].width);
                pBoundaryTop = boundaryTop;
            }
            if( prm[tpi].boundaryBottom ) {
                memcpy(boundaryBottom, prm[tpi].boundaryBottom, prm[tpi].width);
                pBoundaryBottom = boundaryBottom;
            }

            /* Test optimized kernel */
            VLIB_profile_start(vlib_KERNEL_OPT);
            VLIB_recursiveFilterVert1stOrder(out, in, prm[tpi].width, prm[tpi].height, prm[tpi].weight,
                                             pBoundaryTop, pBoundaryBottom, scratch);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_recursiveFilterVert1stOrder_cn(out_cn, in, prm[tpi].width, prm[tpi].height, prm[tpi].weight,
                                                pBoundaryTop, pBoundaryBottom, scratch);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_recursiveFilterVert1stOrder");
    } else {
        VLIB_recursiveFilterVert1stOrder_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_recursiveFilterVert1stOrder_d.c                      */
/* ======================================================================== */
