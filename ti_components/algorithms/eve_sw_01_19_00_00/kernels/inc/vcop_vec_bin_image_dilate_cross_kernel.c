...
#pragma DATA_SECTION(__pblock_vcop_vec_bin_image_dilate_cross, ".vcop_parameter_block")
#pragma DATA_ALIGN(__pblock_vcop_vec_bin_image_dilate_cross, __ALIGNOF__(int));
unsigned short __pblock_vcop_vec_bin_image_dilate_cross[28];
   ...
   unsigned short pblock[])
{
   ...
   #pragma VCC_VREG("R6", 19);
   #pragma VCC_VREG("R7", 20);
   #pragma VCC_VREG("R8", 21);
   #pragma VCC_VREG("R9", 22);
   #pragma VCC_VREG("in00", 23);
   #pragma VCC_VREG("in01", 24);
   #pragma VCC_VREG("in10", 25);
   #pragma VCC_VREG("in11", 26);
   #pragma VCC_VREG("in20", 27);
   #pragma VCC_VREG("in21", 28);
   ...
   __vcop_vload(__vcop_sizeWU(), __vcop_npt(), _PREG(18), _AGEN(0), _VREG(27), __vcop_alws());
   __vcop_vload(__vcop_sizeWU(), __vcop_npt(), _PREG(20), _AGEN(0), _VREG(28), __vcop_alws());
   __vcop_vshf(_VREG(23), _VREG(17), _VREG(19));
   __vcop_vshf(_VREG(24), _VREG(21), _VREG(20));
   __vcop_vshfor(_VREG(25), _VREG(17), _VREG(19));
   __vcop_vshfor(_VREG(26), _VREG(21), _VREG(20));
   __vcop_vshfor(_VREG(25), _VREG(16), _VREG(19));
   __vcop_vshfor(_VREG(26), _VREG(22), _VREG(20));
   __vcop_vshfor(_VREG(27), _VREG(17), _VREG(19));
   __vcop_vshfor(_VREG(28), _VREG(21), _VREG(20));
   __vcop_vor3(_VREG(25), _VREG(20), _VREG(19));
   __vcop_vstore(__vcop_unconditional(), __vcop_sizeWU(), __vcop_npt(), __vcop_alws(), _VREG(19), _PREG(22), _AGEN(0), _PREG(0));
   __vcop_vloop_end(1u);
   ...
   unsigned short height)
{
   ...
   vcop_vec_bin_image_dilate_cross_init(pIn, out, cols, pitch, height, __pblock_vcop_vec_bin_image_dilate_cross);

   VCOP_BUF_SWITCH_SET(WBUF_VCOP, IBUFHB_VCOP, IBUFLB_VCOP,
                   IBUFHA_VCOP, IBUFLA_VCOP);
   vcop_vec_bin_image_dilate_cross_vloops(__pblock_vcop_vec_bin_image_dilate_cross);
   ...
   unsigned short pblock[])
{
   VCOP_BUF_SWITCH_SET(WBUF_SYST, IBUFHB_SYST, IBUFLB_SYST,
                   IBUFHA_SYST, IBUFLA_SYST);
   vcop_vec_bin_image_dilate_cross_init(pIn, out, cols, pitch, height, pblock);

   VCOP_BUF_SWITCH_SET(WBUF_VCOP, IBUFHB_VCOP, IBUFLB_VCOP,
                   IBUFHA_VCOP, IBUFLA_VCOP);
   vcop_vec_bin_image_dilate_cross_vloops(pblock);
