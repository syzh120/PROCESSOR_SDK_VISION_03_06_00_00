function MMU_init()
{
    ...
    // Set the descriptor for each entry in the address range
    for (var i=0x80200000; i < 0xA0000000; i = i + 0x00200000)
    {
        // Each 'BLOCK' descriptor entry spans a 2MB address range
        MMU.setSecondLevelDescMeta(i, i, attrs);
    }
    ...
    for (var i = 0; i < Program.cpu.memoryMap.length; i++)
    {
        ...
        if (memSegment.name == "CUSTOM_HOST_NDKBUF_MEM")
        {
            ...
            attrs3.shareable = 0;                      // shareable
            attrs3.accPerm = 0;                        // read/write at PL1
            attrs3.attrIndx = 3;                       // MAIR0 byte 3 describes memory attributes for this level2 entry
    ...
    for (var i=0x80000000; i < 0x80200000; i = i + 0x00200000)
    {
        // Each 'BLOCK' descriptor entry spans a 2MB address range
        MMU.setSecondLevelDescMeta(i, i, attrs);
    }
