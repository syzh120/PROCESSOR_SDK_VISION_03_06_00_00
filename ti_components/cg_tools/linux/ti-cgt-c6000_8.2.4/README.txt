...
As of the 8.2.1 release, the DefectHistory.txt file has been replaced with
the two files Open_defects.html and Closed_defects.html. These show the open and
closed defects for the 8.2.x compiler branch. For open bugs, a status of 
...
v8.2 no longer supports the COFF object file format.  Only ELF is supported.  
...
v8.2 no longer supports older CPUs.  v8.2 supports -mv6400+ (-mv6400p), 
...
The C6000 CGT v8.2 is a *NEW* compiler:
* v8.2 will support C6400+, C6740, and C6600 in ELF EABI mode only
...
* C++ object code generated by compilers older than v8.0 is *NOT* compatible
  with v8.x RTS object libraries

You should use v8.2 if you are:
...
As of the 8.2.2 release, a new compiler option, --legacy, has been added.
When legacy code bases tuned for C6000 CGT v7.4.x or older exhibit performance
degradations (cycles or code size) with later compiler versions, this option
may help. Do not use this option in new applications. Any new features added
to newer tool chains will not be available when the --legacy option is used. 
